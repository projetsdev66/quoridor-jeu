...
export function Board({ gameState, localPlayer, mode, showPath, confirmWalls = true, reducedMotion = false, onMove, onWall, onInvalidAction }: BoardProps) {
  ...
  const isMyTurn = gameState.turn === localPlayer && !gameOver && !gameState.ranking.includes(localPlayer);
  // Players who have already reached their goal are parked on the board but no longer
  // count as obstacles — otherwise, in "Centre" mode especially, the first arrival would
  // permanently occupy the target cell and lock everyone else out of finishing.
  const opponentPositions = activePlayers(gameState)
    .filter((player) => player !== localPlayer && !gameState.ranking.includes(player))
    .map((player) => gameState.pos[player]);
  ...
  return (
    <div
      className={`relative mx-auto aspect-square w-[min(94vw,460px)] max-w-[calc(100vw-1.5rem)] shrink-0 overflow-visible rounded-[26px] bg-[var(--color-wood-dark)] p-[3%] shadow-[0_24px_70px_rgba(0,0,0,0.42)] lg:w-[min(46vw,520px)] lg:max-w-[520px] ${isShaking ? 'animate-board-shake' : ''}`}
    ...
    >
      ...
      <div className="relative h-full w-full overflow-hidden rounded-[20px] bg-[#eadfc4] shadow-[inset_0_0_24px_rgba(0,0,0,0.45)]">
        ...
        {activePlayers(gameState).map((player) => (
          <Pawn
            ...
            isActive={gameState.turn === player && !gameOver && !gameState.ranking.includes(player)}
            reducedMotion={reducedMotion}
          />
        ))}
      ...
      <AnimatePresence>
        {pendingWall && isMyTurn && confirmWalls && (
          <div className="absolute bottom-2 left-0 right-0 z-50 flex translate-y-full justify-center pt-2">
            <button
              className="max-w-[calc(100vw-2rem)] rounded-full bg-[var(--color-brass)] px-5 py-2 text-sm font-bold text-[#180f0a] shadow-lg transition-transform active:scale-95 sm:px-6 sm:text-base"
