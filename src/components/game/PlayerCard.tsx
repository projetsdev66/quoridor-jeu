...
export function PlayerCard({ player, name, color, wallsLeft, isActive, isLocal, turnSecondsLeft, turnIsUrgent, turnDuration = TURN_DURATION, avatarLabel, wallCapacity = 10, finishedRank, compact = false }: PlayerCardProps) {
  ...
  return (
    <div
      className={`relative ${compact ? 'min-h-[58px] rounded-xl p-2 sm:p-2.5' : 'min-h-[76px] rounded-2xl p-3 sm:p-4'} overflow-hidden transition-all duration-300 ${
        isActive
    ...
    >
      ...
      <div className={`relative flex items-center ${compact ? 'gap-2' : 'gap-3'}`}>
        <div
          className={`${compact ? 'flex h-8 w-8 text-[10px]' : 'flex h-11 w-11 text-sm'} items-center justify-center rounded-full border border-white/10 font-bold text-white shadow-inner`}
        ...
        <div className="min-w-0 flex-1">
          <div className={`flex items-center justify-between gap-2 font-serif font-bold text-[var(--color-ivory)] ${compact ? 'text-sm' : 'text-lg'}`}>
          ...
          <div className={`${compact ? 'mt-1' : 'mt-2'} flex items-center gap-1`}>
            ...
            {Array.from({ length: wallCapacity }).map((_, i) => (
              <div
                key={i}
                className={`${compact ? 'h-2.5 w-1.5' : 'h-3 w-2'} rounded-sm transition-all duration-300`}
                style={i < wallsLeft ? { backgroundColor: color, boxShadow: '0 1px 2px rgba(0,0,0,0.4)' } : { backgroundColor: '#180f0a', opacity: 0.3 }}
              />
            ))}
            <span className={`${compact ? 'ml-1 text-[10px]' : 'ml-2 text-xs'} font-mono text-[var(--color-ivory)]/65`} aria-label={`${wallsLeft} murs restants`}>{compact ? wallsLeft : `${wallsLeft} murs`}</span>
          </div>
        </div>
      </div>
