  ...
  return (
    <motion.button
      ...
      className={cn(buttonVariants({
        ...
        size: 'lg',
        className: 'brass-ring-hover w-full min-w-0 justify-between rounded-xl p-3.5 text-left font-sans whitespace-normal shadow-[0_12px_30px_-20px_rgba(0,0,0,0.9)]',
      }))}
    >
  ...
  return (
    <motion.button
      ...
      className={cn(buttonVariants({
        variant: 'wood',
        size: 'lg',
        className: 'brass-ring-hover flex min-w-0 flex-col items-start gap-1.5 rounded-xl p-3 text-left font-sans whitespace-normal shadow-[0_12px_30px_-20px_rgba(0,0,0,0.9)]',
      }))}
    >
...
export function MainMenu({
  ...
  return (
    <div className="relative mx-auto w-full max-w-md px-3 py-4 sm:max-w-lg sm:p-6">
      ...
      <div className="relative min-h-[300px] overflow-hidden rounded-2xl border border-[#3b2419] bg-[linear-gradient(145deg,var(--color-wood-dark),#21140d_60%,#180f0a)] p-4 shadow-[0_24px_70px_-24px_rgba(0,0,0,0.9)] before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-[var(--color-brass)]/60 before:to-transparent before:content-[''] sm:p-6">
        <AnimatePresence mode="wait">
          {view === 'main' && (
            <motion.div key="main" variants={variants} initial="initial" animate="animate" exit="exit" className="flex flex-col gap-3">
              ...
              <div className="mt-1 flex items-center gap-3 px-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--color-ivory)]/35">
                <span>Autres façons de jouer</span>
                <span className="h-px flex-1 bg-[#5c3a24]" />
              </div>
          ...
          {view === 'local' && (
            <motion.div key="local" variants={variants} initial="initial" animate="animate" exit="exit" className="flex flex-col gap-4">
                ...
                <div className="rounded-xl border border-[#3b2419] bg-[#180f0a] p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={cn(buttonVariants({ variant: localFormat === 'standard' ? 'outline' : 'ghost', size: 'sm', className: 'h-auto min-w-0 flex-col items-start justify-start whitespace-normal rounded-lg border-[#5c3a24] px-2 py-2 text-left leading-tight' }))}
                    >
                    ...
                    <button
                      ...
                      className={cn(buttonVariants({ variant: localFormat === 'center' ? 'outline' : 'ghost', size: 'sm', className: 'h-auto min-w-0 flex-col items-start justify-start whitespace-normal rounded-lg border-[#5c3a24] px-2 py-2 text-left leading-tight' }))}
                    >
              ...
              <motion.button
                ...
                className={cn(buttonVariants({ variant: 'brass', size: 'lg', className: 'w-full rounded-xl' }))}
              >
          ...
          {view === 'multi' && (
            <motion.div key="multi" variants={variants} initial="initial" animate="animate" exit="exit" className="flex flex-col gap-4">
                ...
                <div className="rounded-xl border border-[#3b2419] bg-[#180f0a] p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={cn(buttonVariants({ variant: roomFormat === 'standard' ? 'outline' : 'ghost', size: 'sm', className: 'h-auto min-w-0 flex-col items-start justify-start whitespace-normal rounded-lg border-[#5c3a24] px-2 py-2 text-left leading-tight' }))}
                    >
                      <span className="block font-bold">Bords</span>
                    ...
                    <button
                      ...
                      className={cn(buttonVariants({ variant: roomFormat === 'center' ? 'outline' : 'ghost', size: 'sm', className: 'h-auto min-w-0 flex-col items-start justify-start whitespace-normal rounded-lg border-[#5c3a24] px-2 py-2 text-left leading-tight' }))}
                    >
                      <span className="block font-bold">Centre</span>
                      <span className="mt-0.5 block text-[10px] opacity-70">Le premier au centre gagne.</span>
                    </button>
              ...
              {error && (
                <div role="alert" className="flex items-center gap-2 rounded-xl border border-red-300/25 bg-red-500/10 px-3 py-2.5 text-xs text-red-200">
                  <AlertCircle className="h-4 w-4 shrink-0 text-red-300" aria-hidden="true" />
                  <span className="min-w-0 flex-1">{error}</span>
                  <button
                    type="button"
                    onClick={() => (joinCode.length === 4 ? handleLookupRoom() : handleCreateRoom())}
                    disabled={loading}
                    className="inline-flex shrink-0 items-center gap-1 rounded-md border border-red-300/25 px-2 py-1 font-bold text-red-100 transition-colors hover:bg-red-300/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-200"
                  >
              ...
              <div>
                ...
                <motion.button
                  ...
                  className={cn(buttonVariants({ variant: 'wood', size: 'md', className: 'w-full rounded-xl border-[#5c3a24]' }))}
                >
          ...
          {view === 'multi-join' && (
            <motion.div key="multi-join" variants={variants} initial="initial" animate="animate" exit="exit" className="flex flex-col gap-4">
              ...
              <motion.button
                ...
                disabled={loading}
                className={cn(buttonVariants({ variant: 'brass', size: 'md', className: 'w-full rounded-xl' }))}
              >
